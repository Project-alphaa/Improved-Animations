...
public class CreeperPartAnimator extends NPCPartAnimator<Creeper, CreeperModel<Creeper>> {
    ...
    public void tick(LivingEntity livingEntity, EntityAnimationData entityAnimationData) {
        ...
        tickAggroTimers(entityAnimationData, livingEntity, TimelineGroupData.INSTANCE.get(ImprovedAnimations.MOD_ID, EntityType.CREEPER, "aggro_start").getFrameLength(), 10);
    }

    @Override
    protected void poseLocatorRig() {
        poseHeadRotation(this.locatorHead);
        poseLookLean(locatorListAll, getTimelineGroup(ImprovedAnimations.MOD_ID, "look_vertical"), getTimelineGroup(ImprovedAnimations.MOD_ID, "look_horizontal"));

        poseWalkCycle();
        poseWalkToStop(locatorListAll, getTimelineGroup(ImprovedAnimations.MOD_ID, "walk_to_stop"));

        poseAggro(locatorListAll, getTimelineGroup(ImprovedAnimations.MOD_ID, "aggro_loop"), getTimelineGroup(ImprovedAnimations.MOD_ID, "aggro_start"));
    }
