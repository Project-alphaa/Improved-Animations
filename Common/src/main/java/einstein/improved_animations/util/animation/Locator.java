...
public class Locator {
    ...
    public void rotatePoseStack(PoseStack poseStack){
        if (this.rotateZ != 0.0F) {
            poseStack.mulPose(new Quaternionf().rotateZ((float)Math.toRadians(this.rotateZ)));
        }

        if (this.rotateY != 0.0F) {
            poseStack.mulPose(new Quaternionf().rotateY((float)Math.toRadians(this.rotateY)));
        }

        if (this.rotateX != 0.0F) {
            poseStack.mulPose(new Quaternionf().rotateX((float)Math.toRadians(this.rotateX)));
        }
    }
