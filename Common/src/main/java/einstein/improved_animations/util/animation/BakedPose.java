...
public class BakedPose {
    ...
    private static PartPose lerpPartPose(float value, PartPose partPoseOld, PartPose partPose){
        ...
        if(Math.abs(partPose.xRot - partPoseOld.xRot) > Mth.PI){
            ImprovedAnimations.LOGGER.warn("Snapping on the X axis of {} degrees", Math.toDegrees(Math.abs(partPose.xRot - partPoseOld.xRot)));
        }
        if(Math.abs(partPose.yRot - partPoseOld.yRot) > Mth.PI){
            ImprovedAnimations.LOGGER.warn("Snapping on the Y axis of {} degrees", Math.toDegrees(Math.abs(partPose.yRot - partPoseOld.yRot)));
        }
        if(Math.abs(partPose.zRot - partPoseOld.zRot) > Mth.PI){
            ImprovedAnimations.LOGGER.warn("Snapping on the Z axis of {} degrees", Math.toDegrees(Math.abs(partPose.zRot - partPoseOld.zRot)));
        }
